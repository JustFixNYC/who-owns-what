WITH TIME_SERIES AS (
    ...
    DOB_VIOLS AS (
      SELECT
          ...
          COUNT(*) FILTER (WHERE VIOLATIONTYPECODE='EGNCY' OR VIOLATIONTYPECODE='IMEGNCY') DOB_VIOLS_EMERGENCY,
          COUNT(*) FILTER (WHERE VIOLATIONTYPECODE!= 'EGNCY' OR VIOLATIONTYPECODE != 'IMEGNCY') DOB_VIOLS_NONEMERGENCY,
          COUNT(*) FILTER (WHERE VIOLATIONTYPECODE IS NOT NULL) DOB_VIOLS_TOTAL
      FROM DOB_VIOLATIONS
      WHERE BBL = %(bbl)s
    ...
    ECB_VIOLS AS (
      SELECT
          TO_CHAR(ISSUEDATE, 'YYYY-MM') AS MONTH,
          COUNT(*) FILTER (WHERE SEVERITY IS NOT NULL) AS ECB_VIOLS_TOTAL
      FROM ECB_VIOLATIONS
      WHERE BBL = %(bbl)s
      AND ISSUEDATE >= '2010-01-01'
      GROUP BY MONTH
  ...
  SELECT 
      ...
      COALESCE(DOB_VIOLS_EMERGENCY, 0) AS DOB_VIOLS_EMERGENCY,
      COALESCE(DOB_VIOLS_NONEMERGENCY, 0) AS DOB_VIOLS_NONEMERGENCY,
      COALESCE(DOB_VIOLS_TOTAL, 0) AS DOB_VIOLS_TOTAL,
      COALESCE(ECB_VIOLS_TOTAL, 0) AS ECB_VIOLS_TOTAL
  FROM TIME_SERIES T
  LEFT JOIN VIOLS V ON T.MONTH = V.MONTH
  LEFT JOIN COMPLAINTS C ON T.MONTH = C.MONTH
  LEFT JOIN PERMITS P ON T.MONTH = P.MONTH
  LEFT JOIN DOB_VIOLS DV ON T.MONTH = DV.MONTH
  LEFT JOIN ECB_VIOLS EV ON T.MONTH = EV.MONTH
