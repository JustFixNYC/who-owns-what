WITH TIME_SERIES AS (
    ...
    DOBVIOLS AS (
      SELECT
          ...
          COUNT(*) FILTER (WHERE VIOLATIONTYPECODE='EGNCY' OR VIOLATIONTYPECODE='IMEGNCY') DOBVIOLS_EMERGENCY,
          COUNT(*) FILTER (WHERE VIOLATIONTYPECODE!= 'EGNCY' OR VIOLATIONTYPECODE != 'IMEGNCY') DOBVIOLS_NONEMERGENCY,
          COUNT(*) FILTER (WHERE VIOLATIONTYPECODE IS NOT NULL) DOBVIOLS_TOTAL
      FROM DOB_VIOLATIONS
      WHERE BBL = %(bbl)s
    ...
    ECBVIOLS AS (
      SELECT
          TO_CHAR(ISSUEDATE, 'YYYY-MM') AS MONTH,
          COUNT(*) FILTER (WHERE SEVERITY IS NOT NULL) AS ECBVIOLS_TOTAL
      FROM ECB_VIOLATIONS
      WHERE BBL = %(bbl)s
      AND ISSUEDATE >= '2010-01-01'
      GROUP BY MONTH
  ...
  SELECT 
      ...
      COALESCE(DOBVIOLS_EMERGENCY, 0) AS DOBVIOLS_EMERGENCY,
      COALESCE(DOBVIOLS_NONEMERGENCY, 0) AS DOBVIOLS_NONEMERGENCY,
      COALESCE(DOBVIOLS_TOTAL, 0) AS DOBVIOLS_TOTAL,
      COALESCE(ECBVIOLS_TOTAL, 0) AS ECBVIOLS_TOTAL
  FROM TIME_SERIES T
  LEFT JOIN VIOLS V ON T.MONTH = V.MONTH
  LEFT JOIN COMPLAINTS C ON T.MONTH = C.MONTH
  LEFT JOIN PERMITS P ON T.MONTH = P.MONTH
  LEFT JOIN DOBVIOLS DV ON T.MONTH = DV.MONTH
  LEFT JOIN ECBVIOLS EV ON T.MONTH = EV.MONTH
