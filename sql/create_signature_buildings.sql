CREATE TEMP TABLE signature_loan_status AS (
	...
	loan_json_agg AS (
		SELECT
			...
			json_agg(link_obj) FILTER (
				WHERE
					link_obj IS NOT NULL
			) AS links
		FROM
			loan_json_rows
		GROUP BY
	...
	SELECT
		bbl,
		json_build_object('actions', actions, 'links', links) AS loan_info
	FROM
		loan_json_agg
);
...
CREATE TEMP TABLE IF NOT EXISTS signature_pluto_geos AS (
	SELECT
		...
		s.loan_info,
		coalesce(
			(s.loan_info -> 'actions' ->> -1) :: json ->> 'action',
			'no_action'
		) AS latest_action,
		p.borocode,
		p.block,
...
CREATE TABLE signature_buildings AS (
	...
	hp_cases AS (
		SELECT
			...
			count(*) FILTER (
				WHERE
					openjudgement = 'YES'
			) AS hp_open_judgements,
			coalesce(
				sum(penalty :: float) FILTER (
					WHERE
						openjudgement = 'YES'
				),
				0
			...
			count(*) FILTER (
				WHERE
					findingofharassment IN ('After Trial', 'After Inquest')
			) AS hp_find_harassment,
			count(*) FILTER (
				WHERE
					casestatus IN ('APPLICATION PENDING', 'PENDING')
			) AS hp_active
		FROM
			hpd_litigations
			LEFT JOIN signature_unhp_buildings AS sb USING(bbl)
	...
	hpd_viol_total AS (
		SELECT
			...
			count(*) FILTER (
				WHERE
					novdescription ~* 'mold'
					OR ordernumber = any('{583,507,878,879,550}')
			) AS hpd_viol_water
		FROM
			hpd_violations
			LEFT JOIN signature_unhp_buildings AS sb USING(bbl)
	...
	hpd_comp AS (
		SELECT
			...
			COUNT(*) FILTER (
				WHERE
					TYPE = ANY('{IMMEDIATE EMERGENCY,HAZARDOUS,EMERGENCY}')
			) AS hpd_comp_emerg_total,
			array_agg(DISTINCT apartment) FILTER (
				WHERE
					apartment NOT IN ('BLDG', 'NA')
			) AS hpd_comp_apts,
			COUNT(*) FILTER (
				WHERE
					majorcategory = ANY('{HEAT/HOT WATER,HEATING}')
					OR minorcategory = ANY('{HEAT/HOT WATER,HEATING}')
			) as hpd_comp_heat,
			COUNT(*) FILTER (
				WHERE
					majorcategory = ANY('{MOLD,WATER LEAK}')
					OR minorcategory = ANY('{MOLD,WATER LEAK}')
			) as hpd_comp_water,
			COUNT(*) FILTER (
				WHERE
					majorcategory = 'PESTS'
					OR minorcategory = 'PESTS'
			) as hpd_comp_pests
		FROM
	...
	hpd_vacate AS (
		SELECT
			...
			concat(
				initcap(vacatetype),
				' (',
				to_char(vacateeffectivedate, 'Mon d, YYYY'),
				')'
			) AS hpd_active_vacate
		FROM
			hpd_vacateorders
			LEFT JOIN signature_unhp_buildings AS sb USING(bbl)
		WHERE
			...
			AND coalesce(bbl, '') != '' -- keep 'entire building' orders over 'partial' ones if both
		ORDER BY
			bbl,
			vacatetype,
	...
	hpd_reg AS (
		SELECT
			bbl,
			CASE
				WHEN count(*) = 1 THEN max(buildingid)
				ELSE NULL
			END AS buildingid
		FROM
			hpd_registrations
	...
	dob_ecb_viol AS (
		SELECT
			...
			) AS dob_ecb_viol_total,
			count(*) FILTER (
				WHERE
					is_active
			) AS dob_ecb_viol_open
		FROM
			dob_ecb_stacked
			LEFT JOIN signature_unhp_buildings AS sb USING(bbl)
		WHERE
	...
	SELECT
		...
		-- EXTERNAL LINKS
		CASE
			WHEN hpd_reg.buildingid IS NOT NULL THEN concat(
				'https://hpdonline.nyc.gov/hpdonline/building/',
				hpd_reg.buildingid
			)
			ELSE concat(
		...
		concat(
			'https://portal.displacementalert.org/property/',
			sp.bbl
		) AS link_dap,
		concat('https://whoownswhat.justfix.org/bbl/', sp.bbl) AS link_wow,
		concat(
			'https://www.mygovnyc.org/?lnglat=',
			sp.longitude,
			'%2C',
			sp.latitude
		) AS link_political,
		...
		round(
			coalesce(array_length(hpd_comp.hpd_comp_apts, 1), 0) :: float / nullif(sp.unitsres, 0) :: float * 100
		) AS hpd_comp_apts_pct,
