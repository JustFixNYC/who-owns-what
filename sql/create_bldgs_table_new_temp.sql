-- This is a tempoary version of the table to use until we 
-- are able to finalize the process for creating the OCA evictions
-- table we use here.

create table wow_bldgs_new_temp as 
...
select distinct on (registrations.bbl)
  ...
  oca_evictions.eviction_filings_since_2017 as evictionfilings
from wow.hpd_registrations_with_contacts as registrations
left join (
  select bbl,
...
left join wow.oca_evictions_bldgs on (registrations.bbl = oca_evictions.bbl);

create index on wow_bldgs_new_temp (registrationid);
create index on wow_bldgs_new_temp (bbl);
