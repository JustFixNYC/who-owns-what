...
dob_comp AS (
    ...
    LEFT JOIN pad_adr USING(bin)
    WHERE GREATEST(dateentered, dispositiondate, inspectiondate) 
        between email_date_start_1_year and email_date_end
    GROUP BY bbl
),
...
SELECT DISTINCT ON (bbl)
    ...
    coalesce(hpd_viol.hpd_viol_bc__6mo, 0)::float 
        / nullif(x.unitsres, 0)::float AS hpd_viol_per_unit_bc__6mo,

    coalesce(hpd_viol.hpd_viol_bc__1mo, 0)::float 
        / nullif(x.unitsres, 0)::float AS hpd_viol_per_unit_bc__1mo,

    coalesce(hpd_viol.hpd_viol_bc__week, 0)::float 
        / nullif(x.unitsres, 0)::float AS hpd_viol_per_unit_bc__week,

    round(
        coalesce(array_length(hpd_viol.hpd_viol_bc_apts__6mo, 1), 0)::float 
        / nullif(x.unitsres, 0)::float * 100
    ) AS hpd_viol_unit_pct_bc__6mo,

    round(
        coalesce(array_length(hpd_viol.hpd_viol_bc_apts__week, 1), 0)::float 
        / nullif(x.unitsres, 0)::float * 100
    ...
    hpd_comp_by_type.hpd_comp__bldg_month_by_type AS hpd_comp__bldg_month_by_type,
    ...
    coalesce(hpd_comp.hpd_comp__6mo, 0)::float 
        / nullif(x.unitsres, 0)::float AS hpd_comp_per_unit__6mo,

    coalesce(hpd_comp.hpd_comp__1mo, 0)::float 
        / nullif(x.unitsres, 0)::float AS hpd_comp_per_unit__1mo,

    coalesce(hpd_comp.hpd_comp__week, 0)::float 
        / nullif(x.unitsres, 0)::float AS hpd_comp_per_unit__week,

    round(
        coalesce(array_length(hpd_comp.hpd_comp_apts__6mo, 1), 0)::float 
        / nullif(x.unitsres, 0)::float * 100
    ) AS hpd_comp_unit_pct__6mo,

    round(
        coalesce(array_length(hpd_comp.hpd_comp_apts__week, 1), 0)::float 
        / nullif(x.unitsres, 0)::float * 100
...
LEFT JOIN hpd_comp_by_type USING(bbl)
