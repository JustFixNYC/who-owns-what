...
export const RentstabSummary: React.FC<{
  ...
  return <p>
    <Trans>
      This portfolio also had an estimated <b>net <Select value={changeType} gain="gain" other="loss" /></b> of <b>{absTotalRsDiff}</b> <Plural value={absTotalRsDiff} one="rent stabilized unit" other="rent stabilized units" /> since 2007 (gained {absTotalRsGain}, lost {absTotalRsLoss}).
    </Trans>
    {" "}
    <Trans>
      This represents <b>{rsProportion}%</b> of the total size of this portfolio.
    </Trans>
    {rsLossAddr && (rsLossAddrDiff < 0) && <Trans> The building that has lost the most units is <b>{rsLossAddr.housenumber} {rsLossAddr.streetname}, {rsLossAddr.boro}</b>, which has lost <b>{absRsLossAddrDiff}</b> <Plural value={absRsLossAddrDiff} one="unit" other="units" /> in the past 10 years.</Trans>}
  </p>;
};
