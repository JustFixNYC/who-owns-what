...
export const ViolationsSummary: React.FC<{
  ...
  const openViolations = _.isNil(openviolationsperresunit)
    ? <Trans>The average number of open HPD violations per residential unit is not available for this portfolio.</Trans>
    : <>
        <Trans>This portfolio has an average of <b>{openviolationsperresunit}</b> open HPD violations per residential unit.</Trans>{" "}
        {(openviolationsperresunit >= VIOLATIONS_AVG - 0.05 && openviolationsperresunit < VIOLATIONS_AVG + 0.05)
  ...
  return (
    <>
      ...
      <p>
        {openViolations}{" "}
        <Trans>According to available HPD data, this portfolio has received <b>{totalviolations}</b> total violations.</Trans>
      </p>
    </>
  );
};
