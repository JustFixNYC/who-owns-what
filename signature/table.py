...
def download_s3_objects(config: SignatureConfig):
    s3 = boto3.client(
        "s3",
...
def create_s3_tables(wow_cur, config: SignatureConfig):
...
def create_derived_tables(wow_cur, config: SignatureConfig):
...
def populate_tables(wow_cur, config: SignatureConfig):
    # Since Signature data is private, if you don't have access, the empty
    # tables won't be populated, and this won't affect WOW tables

    print(f"Creating Signature tables")
    create_s3_tables(wow_cur, config)

    if not config.has_s3_creds and not config.is_testing:
        print("No AWS keys to access Signature files in S3. Leaving tables empty")
        return

    if not config.is_testing:
        download_s3_objects(config)

    print(f"Populating Signature tables")
    ...
    create_derived_tables(wow_cur, config)
